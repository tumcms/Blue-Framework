...
enum ePaddingStrategy { Mirror };

enum eFilterSize { Small = 3, Medium = 5, Large = 7 };
...
inline buw::Image<buw::Color<T, N>> applyGaussian(buw::Image<buw::Color<T, N>> image, buw::Matrix22f sigma) {
	...
	int offset = 2;
	switch (filterSize) {
	case eFilterSize::Small: offset = 2; break;
	case eFilterSize::Medium: offset = 1; break;
	case eFilterSize::Large: offset = 0; break;
...
inline buw::Image<buw::Color<T, N>> createPaddedImage(buw::Image<buw::Color<T, N>> image, eFilterSize filterSize) {
...
BLUE_BLUEFRAMEWORK_IMAGEPROCESSING_EMBED_INTO_BUW_NAMESPACE(applyGaussian)
