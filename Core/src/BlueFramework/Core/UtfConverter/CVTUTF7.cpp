...
tabinit()
{
	...
	limit = strlen(base64);
	for (i = 0; i < limit; ++i)
		invbase64[base64[i]] = static_cast<short>(i);
                ...
                int optional_, int verbose)
{
	...
	do
	{
		...
		if (shifted)
		{
			...
			{
				int n = 6;
				int read_n_bit_pos = ((buffertemp = (BITbuffer >> (32-(n)))), (BITbuffer <<= (n)), (bufferbits -= (n)), buffertemp);

				*target++ = base64[read_n_bit_pos];
                ...
                UCS2** targetStart, UCS2* targetEnd)
{
	ConversionResult result = ok;
	DECLARE_BIT_BUFFER;
	int shifted = 0, first = 0, wroteone = 0, base64EOF, base64value = 0, done;
	...
	do
	{
		...
		if (shifted)
		{
			...
			{
				/* write a unicode */
				*target++ = static_cast<UCS2>( READ_N_BITS(16)  );
				wroteone = 1;
			}
