...
public:
	Viewport(const buw::eRenderAPI renderAPI) {
		...
		std::stringstream ss;
		ss << "MSAA Unit Test (" << renderSystem_->getName() << ")";
...
TEST(Engine, ClearScreen_Direct3D11) {
	runMSAAUnitTest(buw::eRenderAPI::Direct3D11);
}

TEST(Engine, ClearScreen_Direct3D12) {
	runMSAAUnitTest(buw::eRenderAPI::Direct3D12);
}

TEST(Engine, ClearScreen_OpenGL) {
	runMSAAUnitTest(buw::eRenderAPI::OpenGL);
}
