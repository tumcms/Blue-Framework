...
public:
	SkyboxEffect(buw::IRenderSystem* renderSystem, buw::ReferenceCounted<buw::IViewport> viewport, buw::ReferenceCounted<buw::IConstantBuffer> worldBuffer)
	    : Effect(renderSystem), viewport_(viewport), worldBuffer_(worldBuffer) {
	}
...
private:
	void v_init() {
		...
		pipelineState_ = createPipelineState(buw::pipelineStateDescription("Shader/skybox.be", "", vertexLayout, buw::ePrimitiveTopology::TriangleList,
		                                                                   {buw::eTextureFormat::R8G8B8A8_UnsignedNormalizedInt_SRGB}, false, false));
